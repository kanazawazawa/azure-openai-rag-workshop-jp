...
5. **OpenAIモデルデプロイメント**: このワークショップでは、Azureにホストされた`gpt-3.5-turbo`モデルを使用します。このコードは、OpenAIのAPIやOllameと最小限の変更で動作するように適応することもできます。
...
- **ジェネレーター**: ライターのように振る舞い、プロンプトと取得した情報を基に応答を作成します。このワークショップでは、OpenAIの`gpt-3.5-turbo`がジェネレーターとして機能します。
