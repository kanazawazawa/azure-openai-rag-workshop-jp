...
本ワークショップで作成するバックエンドの Web API は **インターネット上に公開された状態** でデプロイされます。  
そのため、**機密データや個人情報を含むデータの使用は厳禁** です。  
独自のデータを使用する場合は、**公開されても問題のないテストデータ** をご利用ください。  
また、貴社のセキュリティポリシーやコンプライアンス要件に従い、適切な環境でご利用ください。

ワークショップ終了後は、不要なコストやセキュリティリスクを回避するため、**必ず作成したAzureリソースを削除** してください。
...
| Azureアカウント        | 必要な方は[無料のAzureアカウントを取得](https://azure.microsoft.com/free) |
...
※本ワークショップで作成される Azure リソースの例。下記リソースが作成可能なサブスクリプションである必要があります。クォータ制限などにお気を付けください。  
- リソースグループ
- Azure OpenAI
...
※事前準備にご不安のある方は、以降の手順の`環境構築`を進めていただき、`azd provision` までエラーなく進めることができればワークショップは問題なく実行できる可能性が高いです。  エラーが発生する場合、よくある原因は、権限不足、もしくはAzureリソースの何れかがクォータ制限やpolicyなどでデプロイできない環境である可能性が高いです。  
...
リソースのデプロイがうまくできない場合もご安心ください。
デプロイが成功している方とペアになるか、講師の画面をご確認いただくことで理解を深めることができると思います。
</div>
...
プロジェクトのルートからこのコマンドを実行して、アプリケーションをビルドしてデプロイします（このコマンドはプロジェクトルートにある`azure.yaml`ファイルにリストされているサービスをデプロイします）:
